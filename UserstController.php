...
class UserstController extends Controller
{
    ...
    public function affiAction(Request $request)
    {
        ...
        $sql = "select user_story_backlog_sprint.id_user_story_bs,user_story_backlog_sprint.id_sprint,user_story_backlog_sprint.description_user_story_bs,sprint.id_sprint,sprint.description,user_story_backlog_produit.id_user_story_backlog_produit,user_story_backlog_produit.user_story_bp,user_story_backlog_produit.ide_backlog_feat,projets.nom_projet,equipe.nom_equipe,backlog_produit.id_backlog_feature,fos_user.id from fos_user  inner join equipe on equipe.ide_scrum_master=fos_user.id  INNER join projets on projets.id_projet=equipe.ide_projet inner join backlog_sprint on projets.id_projet=backlog_sprint.id_projet inner join backlog_produit on projets.id_projet=backlog_produit.ide_projet and equipe.ide_projet=projets.id_projet INNER JOIN  user_story_backlog_produit on user_story_backlog_produit.ide_backlog_feat=backlog_produit.id_backlog_feature inner join sprint on sprint.id_bs=backlog_sprint.id_bs inner join user_story_backlog_sprint ON user_story_backlog_sprint.id_sprint=sprint.id_sprint where fos_user.username=:dom";
        $params[':dom'] = $user;
        $em = $this->getDoctrine()->getManager();
    ...
    {
        $em = $this->getDoctrine()->getManager();
        $sql = "select user_story_backlog_produit.id_user_story_backlog_produit,user_story_backlog_produit.user_story_bp,user_story_backlog_produit.ide_backlog_feat,backlog_produit.id_backlog_feature from fos_user inner join equipe on equipe.ide_scrum_master=fos_user.id INNER join projets on projets.id_projet=equipe.ide_projet inner join backlog_sprint on projets.id_projet=backlog_sprint.id_projet inner join backlog_produit on projets.id_projet=backlog_produit.ide_projet and equipe.ide_projet=projets.id_projet INNER JOIN user_story_backlog_produit on user_story_backlog_produit.ide_backlog_feat=backlog_produit.id_backlog_feature where fos_user.username=:dom and user_story_backlog_produit.etat=0";
        $params[':dom'] = $k;
        $em = $this->getDoctrine()->getManager();
        $stmt = $em->getConnection()->prepare($sql);
        $stmt->execute($params);
